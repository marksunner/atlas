...
#define THREADS_PER_OUT (BLOCK_SIZE / N_PER_BLOCK) // 64
...
// ── Fused Gate+Up 2x with shared expert — K=2 batch variant (wide) ──
//
// Grid: (ceil(N/8), 2*(top_k+1), 2)  Block: (256, 1, 1)
extern "C" __global__ void moe_expert_gate_up_shared_batch2(
    ...
    const unsigned int warp_in_output = lane / WARP_SIZE; // 0 or 1
    ...
    __shared__ float smem_reduce[N_PER_BLOCK * 4]; // [4 outputs × (2 warps × 2 accs)]
    ...
    // Cross-warp reduction via shared memory (2 warps per output)
    if (warp_lane == 0) {
        smem_reduce[local_out * 4 + warp_in_output * 2]     = acc1;
        smem_reduce[local_out * 4 + warp_in_output * 2 + 1] = acc2;
    }
    __syncthreads();

    // First thread of each output writes final result
    if (lane == 0) {
        float final1 = smem_reduce[local_out * 4] + smem_reduce[local_out * 4 + 2];
        C[n1] = __float2bfloat16(final1);
        if (have_n2) {
            float final2 = smem_reduce[local_out * 4 + 1] + smem_reduce[local_out * 4 + 3];
            C[n2] = __float2bfloat16(final2);
        }
    }
}

// ── Fused SiLU+Down 2x with shared expert — K=2 batch variant (wide) ──
//
// Grid: (ceil(N/8), 2*(top_k+1), 1)  Block: (256, 1, 1)
extern "C" __global__ void moe_expert_silu_down_shared_batch2(
    ...
    __shared__ float s_lut[16];
    __shared__ float s_act[1280]; // SiLU(gate)*up precomputed for reuse across outputs
    ...
    if (lane == 0) {
        ...
        if (have_n2) {
            float final2 = smem_reduce[local_out * 4 + 1] + smem_reduce[local_out * 4 + 3];
            out[n2] = __float2bfloat16(final2);
        }
    }
}
