...
impl MoeLayer {
    ...
    pub fn forward_k3(
        ...
        if let (Some(gp), Some(up), Some(dp), Some(sh)) = (
            ...
            self.clamp_routed_gate_up(ctx.gpu, expert_gate_out, expert_up_out, 3 * top_k * inter, stream)?;
            self.clamp_shared_gate_up(ctx.gpu, shared_gate_scratch, shared_up_scratch, 3 * inter, stream)?;
