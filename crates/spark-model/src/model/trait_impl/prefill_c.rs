...
impl TransformerModel {
    pub(super) fn prefill_twophase_dispatch(
        ...
        // Sliding split pool: twin uploads (same pattern as prefill_b's
        // upload_meta / upload_paged — see sliding_meta.rs for the layout).
        let ring_len = self.sliding_ring_len();
        let mut sliding_bt_dev = DevicePtr::NULL;
        if ring_len > 0 {
            if needs_paged {
                let current_blocks = seq.block_table.len();
                let meta_ref = seq.chunked_prefill_meta.as_ref().unwrap();
                let sliding_upload_start = meta_ref.uploaded_sliding_blocks;
                sliding_bt_dev = meta_ref.sliding_block_table;
                if sliding_upload_start < current_blocks {
                    ...
                    self.gpu.copy_h2d_async(
                        bytes,
                        sliding_bt_dev.offset(sliding_upload_start * std::mem::size_of::<u32>()),
                        stream,
                    )?;
        ...
        let attn_metadata = AttnMetadataDev {
            ...
            sliding_block_table: sliding_bt_dev,
        };
