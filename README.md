...
| StepFun | Step 3.7 Flash | `stepfun-ai/Step-3.7-Flash-NVFP4` | 196B / ~11B | Full + sliding attention, 288-expert sigmoid MoE, MTP K=3 |
...
Per-model recipes (vision input, multi-node EP=2, Step 3.7 Flash setup, single-GPU 122B with the tighter budget) live in [`QUICKSTART.md`](QUICKSTART.md) and [`docs/DEPLOYMENT.md`](docs/DEPLOYMENT.md). Build-from-source instructions are in [`CONTRIBUTING.md`](CONTRIBUTING.md), and the kernel build pipeline is documented in [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md#build-pipeline).
